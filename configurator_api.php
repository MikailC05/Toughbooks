...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_laptop'])) {
    ...
    if ($modelCode === '') {
        ...
        try {
            // Zoek de configuratie in de JSON
            $configFound = null;
            $modelName = '';
            ...
            if (!$configFound) {
                $error = 'Modelcode niet gevonden in configuraties!';
            } else {
                // Check of laptop al bestaat
                ...
                if ($exists->fetch()) {
                    ...
                    // Genereer naam op basis van specs
                    $nameParts = [$modelName];
                    ...
                    if (isset($configFound['specs']['cellular']) && $configFound['specs']['cellular'] !== 'None') {
                        $nameParts[] = $configFound['specs']['cellular'];
                    }
                    ...
                    if (isset($configFound['specs']) && !empty($configFound['specs'])) {
                        ...
                        foreach ($configFound['specs'] as $key => $value) {
                            $specStmt->execute([$laptopId, $key, $value, 'text']);
                        }
                    }
                    
                    // Zorg voor standaard scores
                    ...
                    $pdo->commit();
                    $message = "Laptop succesvol toegevoegd! (Model: $name)";
                }
            }
        } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_laptop'])) {
    ...
    if ($name === '') {
        ...
        try {
            ...
            // Check duplicate
            $exists = $pdo->prepare('SELECT id FROM laptops WHERE model_code = ? AND id <> ? LIMIT 1');
            $exists->execute([$normalizedModel, $laptopId]);
            if ($exists->fetch()) {
                $error = 'Deze modelcode is al in gebruik bij een andere laptop.';
            } else {
...
if (isset($_GET['delete_laptop'])) {
    ...
    try {
        $pdo->beginTransaction();
        $pdo->prepare('DELETE FROM laptop_specs WHERE laptop_id = ?')->execute([$lid]);
...
<div id="addLaptopModal" class="modal">
    <div class="modal-content modal-wide">
        ...
        <div class="info mb-20">
            ℹ️ <strong>Selecteer een modelcode uit de lijst</strong> - de specs en beschrijving worden automatisch ingevuld!
        </div>

        <form method="post">
            <div class="form-group">
                <label>Selecteer Model & Configuratie *</label>
                <select name="laptop_model_code" required style="width: 100%; padding: 0.75rem; font-size: 0.95rem;">
                ...
                <div class="helper-text">
                    Kies het exacte model met de gewenste specificaties. De modelcode bepaalt welke specs de laptop heeft.
                </div>
            </div>
